...
class S2GeoDataModule(pl.LightningDataModule):
    def __init__(
        ...
        if transform=='pretrained': #go this way
    ...
    def setup(self, stage="fit"):
        ...
        #                 mode=self.mode)
        dataset = S2Geo_w_filename(root=self.data_dir, 
                        transform=self.train_transform, 
                        mode=self.mode
                        )
...
class S2Geo(NonGeoDataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Tensor]:
        ...
        if self.transform is not None: #go this way
...
class S2Geo_w_filename(NonGeoDataset):
    """S2-100K dataset.
    ...
    def __getitem__(self, index: int) -> Dict[str, Tensor]:
        ...
        return sample,self.filenames[index]
